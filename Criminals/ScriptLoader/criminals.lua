...
local SCRIPT = scriptLoader.DefineScript() --This defines the SCRIPT object
SCRIPT.ID = "criminals" --This is the ID of your script, it can be any string you want as long as it's unique to this script only.
SCRIPT.Version = "1.1"
SCRIPT.Name = "Criminals" --The name of your script, any name will do. Not required
SCRIPT.Author = "Skvysh, Converted to Scriptloader by Quickstraw" --Who is the author of the script. Not required.
SCRIPT.Desc = "Expands the bounty system." --The description of what the script will do. Not required.
...
Methods.UpdateBounty = function(pid) -- display global messages if needed when a criminal level changes
    ...
    local criminal = Methods.GetNewCriminalLevel(pid)
        ...
        if killerPid ~= -1 then -- if a killer was found
            if bountyItem ~= "" then
                ...
                if itemCount ~= 0 then -- if the player actually has gold
                    ...
                    Methods.GetNewCriminalLevel(pid)
                end
            end
        end
    end
...
SCRIPT:AddHook("OnPlayerEndCharGen", "criminals_creation", function(pid)
    Methods.OnAccountCreation(pid)
end)

SCRIPT:AddHook("OnPlayerLoginFinish", "criminals_login", function(pid)
    Methods.OnLogin(pid)
end)

SCRIPT:AddHook("OnPlayerBounty", "criminals_update_bounty", function(pid)
    Methods.UpdateBounty(pid)
end)

SCRIPT:AddHook("OnPlayerDeath", "criminals_bounty_reward", function(pid)
	tes3mp.LogMessage(enumerations.log.INFO, "Running criminals.ProcessBountyReward()")
	local msg = "zoop\n"
	tes3mp.SendMessage(0, msg, true)
	if tes3mp.DoesPlayerHavePlayerKiller(pid) then
        local killerPid = tes3mp.GetPlayerKillerPid(pid)
		if pid ~= killerPid then
			Methods.ProcessBountyReward(pid, killerPid)
		end
	end
end)
